...
//
//  LoginViewContrlller.swift
//  code-scanning-test
//
//  Created by dustinlikens on 8/11/25.
//
...
class MyChartLoginViewController: UIViewController, UITextFieldDelegate, IWPAuthenticationDelegate {
    ...
    @IBAction func callMyChartHelp(_ sender: Any) {
        if let url = URL(string: "tel://206-987-4444") {
    ...
    @IBAction func emailMyChartHelp(_ sender: Any) {
        ...
        self.presentEmailSelector(to: "mychart@seattlechildrens.org", subject: nil, body: nil, sourceView: sender as! UIView)
        
        recordEvent(type: .click, properties: [.name: "MyChart Help Email", .currentScreen: "Login"])
    }
    
    @IBAction func faqClicked(_ sender: Any) {
        if let url = URL(string: "https://mychart.seattlechildrens.org/Mychart/Authentication/Login?mode=stdfile&option=faq") {
